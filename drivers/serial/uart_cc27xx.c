...
#define DT_DRV_COMPAT ti_cc27xx_uart
...
#define UART_CC27_REG_GET(base, offset) ((base) + (offset))
/*
...
enum uart_cc27xx_pm_locks {
	UART_CC27XX_PM_LOCK_TX,
	UART_CC27XX_PM_LOCK_RX,
	UART_CC27XX_PM_LOCK_COUNT,
};

struct uart_cc27xx_data {
	...
	ATOMIC_DEFINE(pm_lock, UART_CC27XX_PM_LOCK_COUNT);
#endif
};

static inline void uart_cc27xx_pm_policy_state_lock_get(struct uart_cc27xx_data *data,
							enum uart_cc27xx_pm_locks pm_lock_type)
{
#ifdef CONFIG_PM_DEVICE
...
static inline void uart_cc27xx_pm_policy_state_lock_put(struct uart_cc27xx_data *data,
							enum uart_cc27xx_pm_locks pm_lock_type)
{
#ifdef CONFIG_PM_DEVICE
...
static int uart_cc27xx_poll_in(const struct device *dev, unsigned char *c)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static void uart_cc27xx_poll_out(const struct device *dev, unsigned char c)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static int uart_cc27xx_err_check(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static int uart_cc27xx_configure(const struct device *dev, const struct uart_config *cfg)
{
	const struct uart_cc27xx_config *config = dev->config;
	struct uart_cc27xx_data *data = dev->data;
...
static int uart_cc27xx_config_get(const struct device *dev, struct uart_config *cfg)
{
	struct uart_cc27xx_data *data = dev->data;
...
static int uart_cc27xx_fifo_fill(const struct device *dev, const uint8_t *buf, int len)
{
	const struct uart_cc27xx_config *config = dev->config;
	int n = 0;
...
static int uart_cc27xx_fifo_read(const struct device *dev, uint8_t *buf, const int len)
{
	const struct uart_cc27xx_config *config = dev->config;
	int c, n;
...
static void uart_cc27xx_irq_tx_enable(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static void uart_cc27xx_irq_tx_disable(const struct device *dev)
{
	...
	uart_cc27xx_pm_policy_state_lock_put(dev->data, UART_CC27XX_PM_LOCK_TX);
}

static int uart_cc27xx_irq_tx_ready(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static void uart_cc27xx_irq_rx_enable(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
	 ...
	 */
	uart_cc27xx_pm_policy_state_lock_get(dev->data, UART_CC27XX_PM_LOCK_RX);
...
static void uart_cc27xx_irq_rx_disable(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;

	UARTDisableInt(config->reg, UART_INT_RX | UART_INT_RT);

	uart_cc27xx_pm_policy_state_lock_put(dev->data, UART_CC27XX_PM_LOCK_RX);
}

static int uart_cc27xx_irq_tx_complete(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static int uart_cc27xx_irq_rx_ready(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static void uart_cc27xx_irq_err_enable(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static void uart_cc27xx_irq_err_disable(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static int uart_cc27xx_irq_is_pending(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
...
static int uart_cc27xx_irq_update(const struct device *dev)
{
	ARG_UNUSED(dev);
	return 1;
}

static void uart_cc27xx_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
					 void *user_data)
{
	struct uart_cc27xx_data *data = dev->data;
...
#if CONFIG_UART_CC27XX_DMA_DRIVEN

static int uart_cc27xx_async_callback_set(const struct device *dev, uart_callback_t callback,
					  void *user_data)
{
	struct uart_cc27xx_data *data = dev->data;
...
static int uart_cc27xx_async_tx(const struct device *dev, const uint8_t *buf, size_t len,
				int32_t timeout)
{
	const struct uart_cc27xx_config *config = dev->config;
	struct uart_cc27xx_data *data = dev->data;
	...
	struct dma_block_config block_cfg_tx = {
		...
		.dest_address = UART_CC27_REG_GET(config->reg, UART_O_DR),
	...
	struct dma_config dma_cfg_tx = {
		...
		.source_data_size = 1,
		.dest_data_size = 1,
		.source_burst_length = UART_CC27_BURST_LEN,
		.dma_callback = NULL,
		.user_data = NULL,
	};
	...
	/* Lock PM */
	uart_cc27xx_pm_policy_state_lock_get(data, UART_CC27XX_PM_LOCK_TX);
...
static int uart_cc27xx_tx_halt(struct uart_cc27xx_data *data)
{
	const struct uart_cc27xx_config *config = data->dev->config;
	...
	if (total_len) {
		...
		/* Unlock PM */
		uart_cc27xx_pm_policy_state_lock_put(data, UART_CC27XX_PM_LOCK_TX);
	} else {
		return -EINVAL;
	}
...
static void uart_cc27xx_async_tx_timeout(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_cc27xx_data *data = CONTAINER_OF(dwork, struct uart_cc27xx_data,
						     tx_timeout_work);

	uart_cc27xx_tx_halt(data);
}

static int uart_cc27xx_async_tx_abort(const struct device *dev)
{
	struct uart_cc27xx_data *data = dev->data;

	k_work_cancel_delayable(&data->tx_timeout_work);

	return uart_cc27xx_tx_halt(data);
}

static int uart_cc27xx_async_rx_enable(const struct device *dev, uint8_t *buf, size_t len,
				       int32_t timeout)
{
	const struct uart_cc27xx_config *config = dev->config;
	struct uart_cc27xx_data *data = dev->data;
	...
	struct dma_block_config block_cfg_rx = {
		.source_address = UART_CC27_REG_GET(config->reg, UART_O_DR),
	...
	/* Lock PM */
	uart_cc27xx_pm_policy_state_lock_get(data, UART_CC27XX_PM_LOCK_RX);
...
static int uart_cc27xx_async_rx_buf_rsp(const struct device *dev, uint8_t *buf, size_t len)
{
	struct uart_cc27xx_data *data = dev->data;
	unsigned int key;
	int ret = 0;
...
static void uart_cc27xx_notify_rx_processed(struct uart_cc27xx_data *data,
					    size_t processed)
{
	struct uart_event evt;
...
static int uart_cc27xx_async_rx_disable(const struct device *dev)
{
	const struct uart_cc27xx_config *config = dev->config;
	struct uart_cc27xx_data *data = dev->data;
	struct dma_status status;
	...
	/* Unlock PM */
	uart_cc27xx_pm_policy_state_lock_put(data, UART_CC27XX_PM_LOCK_RX);

	if (dma_get_status(config->dma_dev, config->dma_channel_rx, &status) == 0 &&
	    status.pending_length) {
		rx_processed = data->rx_len - status.pending_length;

		uart_cc27xx_notify_rx_processed(data, rx_processed);
	}
...
static void uart_cc27xx_isr(const struct device *dev)
{
	struct uart_cc27xx_data *data = dev->data;
#if CONFIG_UART_CC27XX_DMA_DRIVEN
	const struct uart_cc27xx_config *config = dev->config;
	...
	if (int_status & UART_INT_RXDMADONE) {
		...
		uart_cc27xx_notify_rx_processed(data, data->rx_len);
		...
		if (data->rx_next_len == 0) {
			...
			dma_reload(config->dma_dev, config->dma_channel_rx,
				   (uint32_t)UART_CC27_REG_GET(config->reg, UART_O_DR),
...
static const struct uart_driver_api uart_cc27xx_driver_api = {
	.poll_in = uart_cc27xx_poll_in,
	.poll_out = uart_cc27xx_poll_out,
	.err_check = uart_cc27xx_err_check,
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	.configure = uart_cc27xx_configure,
	.config_get = uart_cc27xx_config_get,
#endif
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = uart_cc27xx_fifo_fill,
	.fifo_read = uart_cc27xx_fifo_read,
	.irq_tx_enable = uart_cc27xx_irq_tx_enable,
	.irq_tx_disable = uart_cc27xx_irq_tx_disable,
	.irq_tx_ready = uart_cc27xx_irq_tx_ready,
	.irq_rx_enable = uart_cc27xx_irq_rx_enable,
	.irq_rx_disable = uart_cc27xx_irq_rx_disable,
	.irq_tx_complete = uart_cc27xx_irq_tx_complete,
	.irq_rx_ready = uart_cc27xx_irq_rx_ready,
	.irq_err_enable = uart_cc27xx_irq_err_enable,
	.irq_err_disable = uart_cc27xx_irq_err_disable,
	.irq_is_pending = uart_cc27xx_irq_is_pending,
	.irq_update = uart_cc27xx_irq_update,
	.irq_callback_set = uart_cc27xx_irq_callback_set,
#endif /* CONFIG_UART_INTERRUPT_DRIVEN */
#if CONFIG_UART_CC27XX_DMA_DRIVEN
	.callback_set = uart_cc27xx_async_callback_set,
	.tx = uart_cc27xx_async_tx,
	.tx_abort = uart_cc27xx_async_tx_abort,
	.rx_enable = uart_cc27xx_async_rx_enable,
	.rx_buf_rsp = uart_cc27xx_async_rx_buf_rsp,
	.rx_disable = uart_cc27xx_async_rx_disable,
#endif /* CONFIG_UART_CC27XX_DMA_DRIVEN */
};
...
#define UART_CC27XX_IRQ_CFG(n)									\
	do {											\
		...
		UARTClearInt(config->reg, UART_INT_RT);						\
												\
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), uart_cc27xx_isr,		\
...
static int uart_cc27xx_init_common(const struct device *dev)
{
	struct uart_cc27xx_data *data = dev->data;
#ifdef CONFIG_UART_CC27XX_DMA_DRIVEN
	const struct uart_cc27xx_config *config = dev->config;
	...
	k_work_init_delayable(&data->tx_timeout_work, uart_cc27xx_async_tx_timeout);

	data->dev = dev;
#endif

#ifdef CONFIG_PM_DEVICE
	atomic_clear_bit(data->pm_lock, UART_CC27XX_PM_LOCK_RX);
	atomic_clear_bit(data->pm_lock, UART_CC27XX_PM_LOCK_TX);
#endif

	/* Configure and enable UART */
	return uart_cc27xx_configure(dev, &data->uart_config);
}

#define UART_CC27XX_INIT_FUNC(n)								\
	static int uart_cc27xx_init_##n(const struct device *dev)				\
	{											\
		struct uart_cc27xx_data *data = dev->data;					\
		int ret;									\
												\
		...
		if (ret) {									\
			return ret;								\
												...
												\
		ret = uart_cc27xx_init_common(dev);						\
		if (ret) {									\
			return ret;								\
		}										\
												\
		/* Enable interrupts */								\
		UART_CC27XX_IRQ_CFG(n);								\
												\
		return 0;									\
	}
...
static int uart_cc27xx_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct uart_cc27xx_config *config = dev->config;
	...
	case PM_DEVICE_ACTION_RESUME:
		return uart_cc27xx_init_common(dev);
	default:
		return -ENOTSUP;
	}
}
...
#define UART_CC27XX_DEVICE_DEFINE(n)								\
												\
	DEVICE_DT_INST_DEFINE(n, uart_cc27xx_init_##n,						\
			      PM_DEVICE_DT_INST_GET(n),						\
			      &uart_cc27xx_data_##n, &uart_cc27xx_config_##n, PRE_KERNEL_1,	\
			      CONFIG_SERIAL_INIT_PRIORITY, &uart_cc27xx_driver_api)

#define UART_CC27XX_INIT(n)									\
	PINCTRL_DT_INST_DEFINE(n);								\
	PM_DEVICE_DT_INST_DEFINE(n, uart_cc27xx_pm_action);					\
	UART_CC27XX_INIT_FUNC(n);								\
												\
	...
	static struct uart_cc27xx_data uart_cc27xx_data_##n = {					\
		...
		UART_CC27XX_INT_FIELDS};							\
												\
	UART_CC27XX_DEVICE_DEFINE(n);

DT_INST_FOREACH_STATUS_OKAY(UART_CC27XX_INIT)

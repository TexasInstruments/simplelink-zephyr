...
static int i2c_cc23x0_transfer(const struct device *dev, struct i2c_msg *msgs,
	...
	{										\
		...
		if (err < 0) {								\
			...
			return err;							\
											...
											\
		data->cfg = i2c_map_dt_bitrate(DT_INST_PROP(id, clock_frequency));	\
		err = i2c_cc23x0_configure(dev, data->cfg | I2C_MODE_CONTROLLER);	\
		if (err) {								\
...
#define CC23X0_I2C(id)									\
	...
	I2C_DEVICE_DT_INST_DEFINE(id,							\
				  ...
				  &i2c_cc23x0_##id##_data,				\
				  &i2c_cc23x0_##id##_config,				\
				  POST_KERNEL,						\

...
#define DT_DRV_COMPAT ti_cc27xx_aes

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(crypto_cc27xx, CONFIG_CRYPTO_LOG_LEVEL);
...
#define CRYPTO_CC27_CAP		(CAP_RAW_KEY | CAP_SEPARATE_IO_BUFS | \
				 CAP_SYNC_OPS | CAP_NO_IV_PREFIX)

/* CCM mode: see https://datatracker.ietf.org/doc/html/rfc3610 for reference */
#define CCM_CC27_MSG_LEN_SIZE_MIN	2
#define CCM_CC27_MSG_LEN_SIZE_MAX	8

#define CCM_CC27_NONCE_LEN_SIZE_MIN	(AES_BLOCK_SIZE - CCM_CC27_MSG_LEN_SIZE_MAX - 1)
#define CCM_CC27_NONCE_LEN_SIZE_MAX	(AES_BLOCK_SIZE - CCM_CC27_MSG_LEN_SIZE_MIN - 1)

#define CCM_CC27_AD_LEN_SIZE		2
#define CCM_CC27_AD_DATA_SIZE_MAX	(AES_BLOCK_SIZE - CCM_CC27_AD_LEN_SIZE)

#define CCM_CC27_TAG_SIZE_MIN		4
#define CCM_CC27_TAG_SIZE_MAX		16

#define CCM_CC27_BYTE_GET(idx, val)	FIELD_GET(0xff << ((idx) << 3), (val))

#define CCM_CC27_B0_GET(ad_len, tag_len, len_size) \
 ...
 */
#define CRYPTO_CC27_BLK_PROC_CYC	23
#define CRYPTO_CC27_BLK_PROC_TIMEOUT	(CRYPTO_CC27_BLK_PROC_CYC << 1)
#define CRYPTO_CC27_OP_TIMEOUT		K_CYC(CRYPTO_CC27_BLK_PROC_TIMEOUT)

#ifdef CONFIG_CRYPTO_CC27XX_DMA_DRIVEN
#define CRYPTO_CC27_OP_TIMEOUT_DMA(len) \
	K_CYC(CRYPTO_CC27_BLK_PROC_TIMEOUT * ((len) / AES_BLOCK_SIZE))

#define CRYPTO_CC27_IS_INVALID_DATA_LEN_DMA(len)	((len) % AES_BLOCK_SIZE)

#define CRYPTO_CC27_REG_GET(offset)	(AES_BASE + (offset))

struct crypto_cc27xx_config {
...
static inline void crypto_cc27xx_pm_policy_state_lock_get(void)
{
...
static inline void crypto_cc27xx_pm_policy_state_lock_put(void)
{
#ifdef CONFIG_PM_DEVICE
...
static void crypto_cc27xx_isr(const struct device *dev)
{
	struct crypto_cc27xx_data *data = dev->data;
	uint32_t status;
...
static void crypto_cc27xx_cleanup(const struct device *dev)
{
#ifdef CONFIG_CRYPTO_CC27XX_DMA_DRIVEN
	const struct crypto_cc27xx_config *cfg = dev->config;
...
static int crypto_cc27xx_ecb_encrypt(struct cipher_ctx *ctx, struct cipher_pkt *pkt)
{
	...
	struct dma_block_config block_cfg_cha = {
		...
		.dest_address = CRYPTO_CC27_REG_GET(AES_O_DMACHA),
	...
	struct dma_block_config block_cfg_chb = {
		.source_address = CRYPTO_CC27_REG_GET(AES_O_DMACHB),
...
#ifdef CONFIG_CRYPTO_CC27XX_DMA_DRIVEN
	if (CRYPTO_CC27_IS_INVALID_DATA_LEN_DMA(pkt->in_len)) {
	...
	crypto_cc27xx_pm_policy_state_lock_get();
	...
	ret = k_sem_take(&data->chb_done, CRYPTO_CC27_OP_TIMEOUT_DMA(pkt->in_len));
	if (ret) {
		goto cleanup;
	}
	...
	while (in_bytes_processed < pkt->in_len) {
		...
		/* Wait for AES operation completion */
		ret = k_sem_take(&data->aes_done, CRYPTO_CC27_OP_TIMEOUT);
		if (ret) {
			goto cleanup;
		}
...
cleanup:
	crypto_cc27xx_cleanup(dev);
	crypto_cc27xx_pm_policy_state_lock_put();
	pkt->out_len = out_bytes_processed;

	return ret;
}

static int crypto_cc27xx_ctr(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	const struct device *dev = ctx->device;
	struct crypto_cc27xx_data *data = dev->data;
...
static int crypto_cc27xx_cmac(struct cipher_ctx *ctx, struct cipher_pkt *pkt,
			      uint8_t *b0, uint8_t *b1)
{
	...
	if (b0) {
		...
		/* Wait for AES operation completion */
		ret = k_sem_take(&data->cha_done, CRYPTO_CC27_OP_TIMEOUT_DMA(AES_BLOCK_SIZE));
		if (ret) {
			goto out;
		}
#else
	...
	/* Wait for AES operation completion */
	ret = k_sem_take(&data->cha_done, CRYPTO_CC27_OP_TIMEOUT_DMA(pkt->in_len));
	if (ret) {
		goto out;
	}
...
static int crypto_cc27xx_ccm_check_param(struct cipher_ctx *ctx, struct cipher_aead_pkt *aead_op)
{
	...
	if (tag_len < CCM_CC27_TAG_SIZE_MIN || tag_len > CCM_CC27_TAG_SIZE_MAX || tag_len & 1) {
		LOG_ERR("CCM parameter invalid (tag_len must be an even value from %d to %d)",
			CCM_CC27_TAG_SIZE_MIN, CCM_CC27_TAG_SIZE_MAX);
		return -EINVAL;
	}

	if (nonce_len < CCM_CC27_NONCE_LEN_SIZE_MIN || nonce_len > CCM_CC27_NONCE_LEN_SIZE_MAX) {
		LOG_ERR("CCM parameter invalid (nonce_len must be a value from %d to %d)",
			CCM_CC27_NONCE_LEN_SIZE_MIN, CCM_CC27_NONCE_LEN_SIZE_MAX);
		return -EINVAL;
	}

	if (ad_len > CCM_CC27_AD_DATA_SIZE_MAX) {
		LOG_ERR("CCM parameter invalid (ad_len max = %d)", CCM_CC27_AD_DATA_SIZE_MAX);
		return -EINVAL;
	}
...
static int crypto_cc27xx_ccm_encrypt(struct cipher_ctx *ctx,
				     struct cipher_aead_pkt *aead_op, uint8_t *nonce)
{
	...
	ret = crypto_cc27xx_ccm_check_param(ctx, aead_op);
	if (ret) {
		return ret;
	}
	 ...
	 * ============================================
	 */
	b0[0] = CCM_CC27_B0_GET(aead_op->ad_len, tag_len, len_size);

	for (i = 0 ; i < sizeof(msg_len) ; i++) {
		byte = CCM_CC27_BYTE_GET(i, msg_len);
	...
	for (i = 0 ; i < sizeof(ad_len) ; i++) {
		byte = CCM_CC27_BYTE_GET(i, ad_len);
		memcpy(&b1[CCM_CC27_AD_LEN_SIZE - 1 - i], &byte, 1);
	}

	memcpy(&b1[CCM_CC27_AD_LEN_SIZE], aead_op->ad, ad_len);
	...
	ret = crypto_cc27xx_cmac(ctx, &data_pkt, b0, b1);
	if (ret) {
		goto cleanup;
	}
	...
	ret = crypto_cc27xx_ctr(ctx, aead_op->pkt, ctri);
	if (ret) {
		goto cleanup;
	}
	...
	ret = crypto_cc27xx_ctr(ctx, &tag_pkt, ctri);
...
static int crypto_cc27xx_ccm_decrypt(struct cipher_ctx *ctx,
				     struct cipher_aead_pkt *aead_op, uint8_t *nonce)
{
	const struct device *dev = ctx->device;
	struct cipher_pkt tag_pkt = { 0 };
...
static int crypto_cc27xx_ctr_drbg(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	const struct device *dev = ctx->device;
	struct crypto_cc27xx_data *data = dev->data;
	uint32_t ctr_len = ctx->mode_params.ctr_info.ctr_len >> 3;
	...
	uint32_t int_flags = AES_IMASK_CHBDONE;
	const struct crypto_cc27xx_config *cfg = dev->config;
	uint32_t trg_count = pkt->out_buf_max / AES_BLOCK_SIZE;
	uint32_t trg_aesop = AES_TRG_AESOP_BUF;

	struct dma_block_config block_cfg_cha = {
		.source_address = (uint32_t)&trg_aesop,
		.dest_address = CRYPTO_CC27_REG_GET(AES_O_TRG),
		.source_addr_adj = DMA_ADDR_ADJ_NO_CHANGE,
		.dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE,
		.block_size = trg_count * sizeof(uint32_t),
	};
	...
	/* Wait for AES operation completion */
	ret = k_sem_take(&data->chb_done, CRYPTO_CC27_OP_TIMEOUT);
	if (ret) {
		goto cleanup;
	}
...
static int crypto_cc27xx_session_setup(const struct device *dev,
				       ...
				       enum cipher_op op_type)
{
	...
	if (op_type == CRYPTO_CIPHER_OP_ENCRYPT) {
		...
		case CRYPTO_CIPHER_MODE_ECB:
			ctx->ops.block_crypt_hndlr = crypto_cc27xx_ecb_encrypt;
			break;
		case CRYPTO_CIPHER_MODE_CTR:
			ctx->ops.ctr_crypt_hndlr = crypto_cc27xx_ctr;
			break;
		case CRYPTO_CIPHER_MODE_CCM:
			ctx->ops.ccm_crypt_hndlr = crypto_cc27xx_ccm_encrypt;
			break;
		case CRYPTO_CIPHER_MODE_CTR_DRBG:
			ctx->ops.ctr_drbg_crypt_hndlr = crypto_cc27xx_ctr_drbg;
			break;
		default:
			return -EINVAL;
		}
		...
		case CRYPTO_CIPHER_MODE_CTR:
			...
			break;
		case CRYPTO_CIPHER_MODE_CCM:
			ctx->ops.ccm_crypt_hndlr = crypto_cc27xx_ccm_decrypt;
			break;
		case CRYPTO_CIPHER_MODE_CTR_DRBG:
			ctx->ops.ctr_drbg_crypt_hndlr = crypto_cc27xx_ctr_drbg;
			break;
...
static int crypto_cc27xx_session_free(const struct device *dev,
				      struct cipher_ctx *ctx)
{
	ARG_UNUSED(ctx);
...
static int crypto_cc27xx_query_caps(const struct device *dev)
{
	ARG_UNUSED(dev);

	return CRYPTO_CC27_CAP;
}

static int crypto_cc27xx_init(const struct device *dev)
{
#ifdef CONFIG_CRYPTO_CC27XX_DMA_DRIVEN
	const struct crypto_cc27xx_config *cfg = dev->config;
#endif
...
static struct crypto_driver_api crypto_enc_funcs = {
	.cipher_begin_session = crypto_cc27xx_session_setup,
	.cipher_free_session = crypto_cc27xx_session_free,
	.query_hw_caps = crypto_cc27xx_query_caps,
};

static struct crypto_cc27xx_data crypto_cc27xx_dev_data;

#ifdef CONFIG_PM_DEVICE

static int crypto_cc27xx_pm_action(const struct device *dev, enum pm_device_action action)
{
	switch (action) {
...
PM_DEVICE_DT_INST_DEFINE(0, crypto_cc27xx_pm_action);

#ifdef CONFIG_CRYPTO_CC27XX_DMA_DRIVEN
static const struct crypto_cc27xx_config crypto_cc27xx_dev_config = {
	.dma_dev = DEVICE_DT_GET(TI_CC27XX_DT_INST_DMA_CTLR(0, cha)),
	.dma_channel_a = TI_CC27XX_DT_INST_DMA_CHANNEL(0, cha),
	.dma_trigsrc_a = TI_CC27XX_DT_INST_DMA_TRIGSRC(0, cha),
	.dma_channel_b = TI_CC27XX_DT_INST_DMA_CHANNEL(0, chb),
	.dma_trigsrc_b = TI_CC27XX_DT_INST_DMA_TRIGSRC(0, chb),
};

DEVICE_DT_INST_DEFINE(0,
		      crypto_cc27xx_init,
		      PM_DEVICE_DT_INST_GET(0),
		      &crypto_cc27xx_dev_data,
		      &crypto_cc27xx_dev_config,
		      POST_KERNEL,

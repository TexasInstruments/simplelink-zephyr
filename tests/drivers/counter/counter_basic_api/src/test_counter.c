...
/* Test function signatures for test_all_instances() */
...
/* Core test runner: runs test func on all devices that pass capability check */
...
static void top_handler(const struct device *dev, void *user_data)
{
	LOG_DBG("%s: top_handler called", dev->name);
...
static void test_set_top_value_with_alarm_instance(const struct device *dev)
{
	...
	/* Wait for 5 wrap-around callbacks */
	if (IS_ENABLED(CONFIG_ZERO_LATENCY_IRQS)) {
		uint32_t timeout_us = 2 * counter_period_us * 5;
		uint32_t elapsed = 0;
		uint32_t poll_interval = 100;

		while (top_cnt < 5U && elapsed < timeout_us) {
			k_busy_wait(poll_interval);
			elapsed += poll_interval;
		}
		top_handler_cnt = top_cnt;
	} else {
		int sem_err = 0;
		k_timeout_t timeout = K_USEC(2 * counter_period_us);

		top_handler_cnt = 0;
		for (int i = 0; i < 5; i++) {
			sem_err = k_sem_take(&top_cnt_sem, timeout);
			if (sem_err != 0) {
				break;
			}
			top_handler_cnt++;
...
/* Alarm callback that validates timing accuracy of interrupt delivery */
...
static void test_all_channels_instance(const struct device *dev)
{
	...
	/* Wait for alarms to fire */
	if (IS_ENABLED(CONFIG_ZERO_LATENCY_IRQS)) {
		uint32_t timeout_us = 2 * counter_period_us * nchan;
		uint32_t elapsed = 0;
		uint32_t poll_interval = 100;

		while (alarm_cnt < (uint32_t)nchan && elapsed < timeout_us) {
			k_busy_wait(poll_interval);
			elapsed += poll_interval;
		}
		cnt = alarm_cnt;
		...
		for (int i = 0; i < nchan; i++) {
			sem_err = k_sem_take(&alarm_cnt_sem, timeout);
			if (sem_err != 0) {
				break;
			}
			cnt++;
	...
	zassert_equal(nchan, cnt,
			"%s: Expecting alarm callback, nchan= %d, cnt = %d", dev->name, nchan, cnt);
...
static void test_short_relative_alarm_instance(const struct device *dev)
{
	...
	for (int i = 0; i < 100; ++i) {
		...
		/*
		 * Wait for callback using semaphore instead of k_busy_wait().
		 * This avoids timing issues due to clock domain differences.
		 * Timeout: 10x tick duration provides generous margin.
		 */
		if (IS_ENABLED(CONFIG_ZERO_LATENCY_IRQS)) {
			uint32_t timeout_us = 10 * tick_us;
			uint32_t elapsed = 0;

			while (alarm_cnt < (uint32_t)(i + 1) && elapsed < timeout_us) {
				k_busy_wait(1);
				elapsed++;
			}
			cnt = alarm_cnt;
			...
			int sem_err;
			k_timeout_t timeout = K_USEC(10 * tick_us);

			sem_err = k_sem_take(&alarm_cnt_sem, timeout);
			if (sem_err != 0) {
				zassert_true(false,
					"%s: Timeout waiting for callback %d (err=%d)",
					dev->name, i + 1, sem_err);
			}
			cnt = i + 1; /* If we got here, callback was received */
		}

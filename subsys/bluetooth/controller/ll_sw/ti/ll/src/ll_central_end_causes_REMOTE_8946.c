...
#include "ll_tx_power.h"
#include "ll_power_control.h"
...
uint8 llSetupNextCentralEvent( void )
{
  ...
  */
  ...
  {
    // For any phy update, if the power control feature is enable we
    // need to signaling we are stopping managing power on the current
    // Phy and moving to managing power on the new phy.
    // More details here - Core 6.0, Vol6, Part B Section 5.1.8
    MAP_LL_PwrCtrl_HandlePhyChangeEvent(connPtr);
  ...
  */
  ...
  // Set the tx power for next Central event
  // If the power control feature is enabled - change to the power
  // Level is set in the power structd
  if (MAP_LL_PwrCtrl_CheckFeatureSupported(connPtr->featureSetInfo.featureSet[LL_FEATURE_BYTE_4]) == LL_STATUS_SUCCESS)
  {
    LL_TxPower_SetTxPowerCmd( (uint32_t *)&linkCmd[connPtr->connId],
                              ((powerControlInfo_t*)connPtr->pPowerControlConnInfo)->txPowerLevelDbm,
                              TX_POWER_DEFAULT_FRACTION );
  }
  else
  {
    // Get the global tx power
    llTxPowerRfCmd_t currentTxPowerRfType = LL_TxPower_GetTxPowerCurrentRfType();

    // Call the LL_TxPower_SetTxPowerCmd
    LL_TxPower_SetTxPowerCmd( (uint32_t *)&linkCmd[connPtr->connId],
                              currentTxPowerRfType.dBm,
                              currentTxPowerRfType.fraction );
  }
    ...
    switch( connPtr->ctrlPktInfo.ctrlPkts[0] )
    {
      ...
      */
      case LL_CTRL_CONNECTION_UPDATE_IND:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_CONNECTION_UPDATE_IND);
      ...
      */
      case LL_CTRL_CHANNEL_MAP_IND:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_CHANNEL_MAP_IND);
      ...
      */
      ...
      case LL_CTRL_ENC_REQ:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_ENC_REQ);
      ...
      */
      ...
      case LL_CTRL_PAUSE_ENC_REQ:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_PAUSE_ENC_REQ);
      ...
      */
      ...
      case LL_CTRL_PAUSE_ENC_RSP:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_PAUSE_ENC_RSP);
      ...
      */
      case LL_CTRL_FEATURE_REQ:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_FEATURE_REQ);
      ...
      */
      case LL_CTRL_VERSION_IND:
        ...
        else // control packet has not been put on the TX FIFO yet
        {
          ...
          // in the of MAP_llSetupCtrlPkt failures
      ...
      case LL_CTRL_PHY_REQ:
        ...
        if ( connPtr->ctrlPktInfo.ctrlPktActive == TRUE )
        {
          if ( TST_FEATURE_FLAG( connPtr->phyInfo.phyFlags, PHY_RSP_RECEIVED ) )
          {
            ...
            MAP_llReplaceCtrlPkt( connPtr, LL_CTRL_PHY_UPDATE_IND,
                                  LL_CTRL_UNDEFINED_PKT );
          }
          // check if this request was rejected
      ...
      */
      case LL_CTRL_PHY_UPDATE_IND:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_PHY_UPDATE_IND );
      ...
      /*
      ** Power Control Request, Response or Indication
      */
      case LL_CTRL_POWER_CONTROL_REQ:
      case LL_CTRL_POWER_CONTROL_RSP:
      case LL_CTRL_POWER_CONTROL_IND:
      {
        return ((uint8_t)MAP_LL_PwrCtrl_HandleProcedure(connPtr, connPtr->ctrlPktInfo.ctrlPkts[0]));
      }
      ...
      */
      case LL_CTRL_LENGTH_RSP:
        ...
        {
          // so try to put it there; being active depends on a success
          connPtr->ctrlPktInfo.ctrlPktActive = MAP_llSetupCtrlPkt( connPtr, LL_CTRL_LENGTH_RSP);

...
#define LL_TASK_PRIORITY                5
#define LL_TASK_STACK_SIZE              1500
#define LL_CUSTOM_INIT_PARAMS           BLE_STACK_USER0_CFG

/** @internal remote threads count */
#define LL_REMOTE_THREAD_COUNT          1
/*******************************************************************************
 * MACROS
 */
 ...
 * LOCAL VARIABLES
 */
static const void *pLlInitParams                     = LL_CUSTOM_INIT_PARAMS;

static SemaphoreP_Handle pInitDoneSemHandle          = NULL;
 ...
 */
__attribute__((weak)) void osalInitTasks( void )
{
  ...
  (void) ICall_enrollService( ICALL_SERVICE_CLASS_BLE, NULL, &entity, &syncHandle );
...
{
  ICall_RemoteTask_t remoteTaskTable;

   remoteTaskTable.imgTaskPriority      = LL_TASK_PRIORITY;
   remoteTaskTable.imgTaskStackSize     = LL_TASK_STACK_SIZE;
   remoteTaskTable.startupEntry         = llStartupEntry;
   remoteTaskTable.ICall_imgInitParam   = (void *) pLlInitParams;
...
uint32 BLE_ServicesInit(const bleServicesParams_t *pServiceParams)
{
  ...
  if ( NULL != pServiceParams )
  {
    ...
    if ( status == SUCCESS )
    {
      if ( pBleServicesParams->syncInitTimeoutTicks != BLE_INIT_NO_WAIT )
      {
        pInitDoneSemHandle = SemaphoreP_createBinary(SemaphoreP_Mode_BINARY);
      }
      ...
      if ( pBleServicesParams->syncInitTimeoutTicks != BLE_INIT_NO_WAIT )
      {
        if (NULL != pInitDoneSemHandle)
        {
          // Wait for init complete. Will be released in init_done callback
          status = SemaphoreP_pend( pInitDoneSemHandle, pBleServicesParams->syncInitTimeoutTicks );

          // Free the resources
          SemaphoreP_delete( pInitDoneSemHandle );
          pInitDoneSemHandle = NULL;
        }
      }
    }
  }
...
uint32 BLE_ServicesParamsInit( bleServicesParams_t *pServiceParams, size_t size )
{
  ...
  if ( ( NULL != pServiceParams ) && ( size == sizeof(bleServicesParams_t) ) )
  {
    ...
    status = HCI_ControllerToHostCallbacksInit( &pServiceParams->hciCbs );
  }
  return status;
}
...
void llInitCompleteNotify( int status )
{
  if (status == SUCCESS)
  {
    if ( ( NULL != pBleServicesParams ) &&
         ( pBleServicesParams->syncInitTimeoutTicks != BLE_INIT_NO_WAIT ) &&
         ( NULL != pInitDoneSemHandle ) )
    {
      SemaphoreP_post( pInitDoneSemHandle );
    }
  }
}
